...
```
## Part 2: Drivers of genetic diversity change
...
```{r}
# Scale and center all variables so that models are comparable:
...
birds$sc_gd <- scale(birds$gene_diversity)
birds$sc_ar <- scale(birds$allelic_richness)

birds$sc_hfi <- scale(birds$hfi)
birds$sc_pop <- scale(log(birds$popden))
## Note: we do not need to scale the 'urban' variable: it is a binary factor
...
Try to test the effects of other urbanization variables on population-specific FST (called *global_fst* in this dataset).

**Note:** there are NA values for FST in this dataset. Start by making a subset of the data where NA values are ommitted, then rescale the variables you need.
...
predictor_list <- as.list(rep('sc_hfi', length(model_list)))

response_list <- list('gene_diversity', 'allelic_richness')
...
## Make the plot:
ggplot(data = plotdata, aes(x = response_var, color = response_var)) + # change as needed
